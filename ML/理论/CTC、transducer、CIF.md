...
- beam search
  - 改进了贪心算法，但是和 CTC 的真正解码方式比，其实还是差的很远——只是对于训得好的 model，这样解码的结果和真正 CTC 解码结果差不太多而已（CTC 已经把绝大部分概率值给了某一个 alignment 了）。beam search 可以对当前所见作 bean，也可以对解码前缀作 beam。详见  https://distill.pub/2017/ctc/ 。

**beam search**

自回归生成模型可以用 beam search，CTC 解码也可以用它。大思路上，都是保持多个候选，具体做法上差异还是有的。自回归的 beam search，本质上是在时间维上展开搜索树，每次保留概率最高的前缀序列。下面看下 CTC 的 beam search。

CTC model 的输出分布是独立的，不能构建搜索树。CTC 的 Beam Search 实质是前缀概率聚合。虽然叫 “beam search”，但它不是“树搜索”，而是概率聚合 + 剪枝的过程。

我们不是逐步扩展「每个前缀所有子节点」，而是在每个时间步 t 维护一批「候选前缀」的累计概率：

$$
P(s, t) = P_b(s, t) + P_{nb}(s, t)
$$

其中：

* ( P_b(s, t) )：到时间步 t，输出序列 s 且最后为 blank 的概率；
* ( P_{nb}(s, t) )：到时间步 t，输出序列 s 且最后为非 blank 的概率。
...
> CTC beam search 不是遍历所有 next-token 组合，
> 而是根据 CTC 的「去重、blank 合并」规则，在动态规划意义上合并路径。——它是“近似动态规划”的一种——确切地说，是“带剪枝的前缀动态规划”。

用 CTC 时，解码签的逐位置的概率分布，一般是 NAR 方式一次生成的。而 CTC beam search 的解码仍然是从左往右，一次扫描过去完成的（而 greedy decoding 则顺序无关）。
