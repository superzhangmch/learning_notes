...
辨析:
- next to = 空间上"挨着". 紧挨着（强调位置相邻）
  - 侧重"在旁边"，静态描述位置关系.
  - 例句: She sat next to me. （她坐在我旁边）/ The bank is next to the supermarket.
- alongside = "并肩"在一起（并排移动或合作）; 并排/沿着旁边（强调平行、并肩）
  - 侧重"与…并列/协同"，常含一起运动或合作的含义.
  - 例句: The boat pulled up alongside the dock. （船停靠在码头旁边）/ She worked alongside her colleagues. （她与同事并肩工作）
...
辨析: in front of vs before
- in front of — 物理空间上的"在…前面". 强调具体位置，看得见摸得着
- before — 抽象意义上的"在…之前/面前". 可表示时间、顺序、正式场合，较书面/正式
- 我坐在电脑前, 用哪个: 电脑是具体物体，你和它之间是物理空间位置关系, 用 in front of
  - before 为什么不行？I'm sitting before the computer. 语法上没错，但听起来非常奇怪/过于古板，像莎士比亚在说话 😄
  - before 表位置时只用于正式/庄重场合, 比如法庭、神坛、权威人物面前, 对着电脑坐着，用它就太违和了
- 一句话记住:
  - 面对人/机器/普通物体 → in front of
  - 面对法官/国王/上帝 → before
...
note: 下面几个, 外国人容易用反, 所以不用学怎么用:
- A compose B (A 组成了 B) : ❌ 非常容易用反	⚠️ 母语者偶尔也会
- A comprise B (A 由 B 组成): ❌❌ 极其容易用反	✅ 母语者基本不会，但会避开
...
- temporarily /ˌtɛmpəˈrɛrəli/ 暂时地（常用度：10）
