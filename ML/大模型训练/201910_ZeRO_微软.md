...
不用 zero 的普通 DP，需要同步的是 fp16 梯度，用的是 ring-all-reduce 这样的操作，这分为两步：scatter-reduce（使得有一个节点有reduce的（这里是梯度累加）最终结果） 与 all-gather（最终结果传播到所有节点）。而用了 zero-2，对梯度作 scatter-reduce 就行了，不需要 all-gather 了。只是需要把 fp16 的参数通过 all-gather 同步下而已。all-gather 增一个减一个，整体上仍然是 scatter_reduce + all_gather，所以通信量不变。
