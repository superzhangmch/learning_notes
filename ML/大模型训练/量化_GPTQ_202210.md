...
OBQ 正是一次只考虑 W 的一行，这样 Wx 是 scalar 数字。下面都假设 W 是行或列向量。
...
注意：
- 它和 OBQ 一次量化一个参数的迭代形式是一样的。推导过程也一模一样。不列。
...
```
    ...
    Hinv = U  # 按算法一，把上三角当做 H⁻¹ 一样用
```

它用 Cholesky 分解得到了更数值稳定的 $H^{-1}$, 并再次用 Cholesky 分解得到上三角矩阵。然后把该上三角矩阵当做 $H^{-1}$ 一样用。详见 paper 算法1.
...
<img width="1380" height="668" alt="image" src="https://github.com/user-attachments/assets/71aeb0eb-3bc2-485a-89b6-d39b7c930718" />
