...
Lucene 的一个原则是只靠倒排索引往外检索。这样即使对于数字，也是当做普通字符串看待，建立的索引。如果要进行数字范围搜索，比如 `100~200` ，那么只能是把 `100~200` 之间的term全找出来，然后对这么多的拉链求并集，才能得到要求的结果。如果范围跨度很大，那么显然很可能会设计到非常多的term。
...
比如 "0xf0000" 拉链本身，就代表了m`0f0000 ~ 0fffff` 这个范围。
