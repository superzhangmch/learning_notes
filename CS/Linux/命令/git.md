...
2). 对一个 repository, 全世界所有 Git commit（理论上）构成一张 DAG
...
# Git :对象、packfile 与 dulwich

  "修改文件"在 Git 里永远是新增:造新的文件内容对象(object) → 新 tree → 新 commit,没改的文件直接复用旧 SHA,旧对象从不改写。

对象的两种存储形态:

- loose objects:一个对象一个文件,简单但数量爆炸。
- packfile(.pack + .idx):许多对象打进一个文件,内部用 delta 压缩(相似对象只存差异),配索引做 SHA → 字节偏移 查找。
  * packfile 不只是把一个文件的多个版本用 delta 做了压缩, 而且把 rep 的多个不相关文件, 也能压缩到一个 packfile.
  * 当pack 中一个文件要修改的时候, 并不是把 pack read-all, partial modify, save back, 而是append或者新packfile 的方式来处理(因为旧版本本来也需要存下来). 

注意区分:delta 压缩省的是体积(同一文件多版本存差异),packing省的是数量(一批对象塞进一个文件)。

分支怎么记:ref 就是"分支名 → commit SHA"。更新分支必须是 compare-and-swap——两个写者同时改一个分支只能成一个,这就是 git push 非快进冲突检测的本质。

**dulwich py lib**: 纯 Python 的 Git 实现(不只是 git 的 py 接口, 而是实现了底层的 git 操作),把上面这些都做好了——对象与 packfile 的读写、delta 解析、commit/merge/diff/log,以及 dulwich.web 的完整 smart HTTP(含 pack negotiation:客户端报 have/want,服务端只回传增量)。需要在 Python 里直接操作 Git 而不调用 git 命令时,用它。
