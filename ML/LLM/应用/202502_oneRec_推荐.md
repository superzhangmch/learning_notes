...
下面讲第一个版本的（但 2025.06 的更详细）：

### 思路总结：

对于被推荐的 item（video item），token 化表示它(通过 RQ-VAE 算法离散化而得到)。从而对于用户的历史消费 items，就可以变成 token list。对它们作 encode 的结果给到一个 T5 一样的 encoder-decoder 结构的 model，由该 model 作自回归生成新的 token list（每个token代表一个 item），完成推荐。
- 所用的大模型，不是预先训好的（通用）模型，而是从零开始训的——毕竟它不使用 text tokens，只使用 item tokens。
- 用户对单个 item 的反馈（时长、点踩）在特征输入层面没有反馈，这个是通过 RLHF-DPO 时的 reward model 使用到的（2025.06 新版中把反馈特征也放入了 encoder 的 input）。
- 所用 model 是 1B 参数的。
...
不用 DPO，而用了 deepseek 的 GRPO。

### item embedding 怎么来的

2025.02 文中没讲。如下图，用对比学习方式学习出（再结合 caption 生辰，联合 loss）。

<img width="1420" height="984" alt="image" src="https://github.com/user-attachments/assets/0f69fce5-eee0-48a0-b752-d658a5e66a13" />

### model_encoder 有了更多的输入维度

2025.02 文中只有item list. 这里则用了丰富的特征，如图：

<img width="954" height="854" alt="image" src="https://github.com/user-attachments/assets/962652cf-fe50-425e-92ee-75c8aed06ac2" />

- **user static pathway**：user identifier (uid), age (age), gender (gender), etc
- **short-term pathway**：video id, author identifiers (aid), tags (tag), timestamps (ts), playtime (playtime), duration (dur), labels (label, user, interactions with each video, including like, follow, forward, dislike, comment, profile entry, etc.)
- **positive-feedback pathway**: 特征来源同 short-term pathway
- **lifelong pathway**: 特征来源同 short-term pathway，但是支持的序列更长，所以要用 QFormer 压缩后才给 encoder。

四种特征见下图：

<img width="1528" height="892" alt="image" src="https://github.com/user-attachments/assets/9a348fee-f27c-42d3-bc8a-b58adc005b2b" />

### inference

infer 时，会作 Pass@K=512 次生成（K越大越好）。然后还要对结果作 reward-model 的优选（OneRec with RM Selection）。根据他的 ab test 表 12 实验，只有这样，才能体现出此文方法的好处（否则，从表格看，好像没啥效果）
