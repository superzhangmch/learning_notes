...
## transformer 不是所有地方都能低精度量化

以下不能。chatgpt 给出

1. Softmax & 注意力概率相关

* **Softmax 输入 (QKᵀ / √d) 的 logits**：
  因为这里的数值动态范围极大，量化会导致数值溢出/塌缩，直接影响注意力分布。
* **Softmax 输出 (attention weights)**：
  要保证概率归一化正确，不能因为量化丢失精度，否则注意力分布完全跑偏。
  👉 实际做法：Q、K 可以低比特存储，但 **QKᵀ 的累积 & Softmax 计算一般在 FP16/FP32**。

2. 归一化 (LayerNorm / RMSNorm)

* 归一化涉及 **方差、均值** 的计算，需要累加/开方，数值稳定性很差。
* 如果量化，容易出现零除、梯度爆炸/消失。
  👉 一般 LayerNorm / RMSNorm 保持 **FP16/FP32**。

3. 残差连接 (Residual Add)

* 两个大向量相加，如果量化可能造成 **累积误差**，残差路径失真，梯度传播会出问题。
  👉 通常 residual add 在高精度。

4. 有些投影层 / 输出层 (比如 logits projection)

* 特别是最后的 **LM Head (hidden → vocab)**：因为词表很大，logits 范围广，若量化过度会严重掉精度。
* 有些方法只对输入 embedding 做量化，输出 projection 仍然 FP16/FP32。
