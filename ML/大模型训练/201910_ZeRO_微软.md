...
Zero-DP 可以优化显存占用，通过把集中式、多副本存放的东西分散到数据并行的多个节点上，但是居然只有 zero-3 才有通信量的一些增加。也就是说，这种优化并没折损什么，完全是不做白不做的。
...
不用 zero 的普通 DP，需要同步的是 fp16 梯度，用的是 ring-all-reduce 这样的操作，这分为两步：scatter-reduce（使得有一个节点有累加最终结果） 与 all-gather（最终结果传播到所有节点）。而用了 zero-2，对梯度作 scatter-reduce 就行了，不需要 all-gather 了。只是需要把 fp16 的参数通过 all-gather 同步下而已。all-gather 增一个减一个，整体上仍然是 scatter_reduce + all_gather，所以通信量不变。
