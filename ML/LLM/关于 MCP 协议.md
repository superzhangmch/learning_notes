...
**简介:**
- server：只是对于各种后端资源做了统一接口的封装。所以 server 其实是薄薄一层。server 不会请求 LLM，当需要的时候，通过反向让 client 用 sampling 来完成。
- client：只是负责与 mcp server 的交互，它暴露给 host 的核接口，都是怎么与 server 打交道的，所以核心接口其实就是：server 的 tool、resource、prompt 三种资源的 list，以及 get。
  - 即暴露给上游 host 的核心接口是： list_resources， list_tools， list_prompts， read_resource， call_tool， call_tool。但是这些接口属于 host 天然需要的，但是它们长什么样（名字、参数等），并不是 mcp 协议的一部分。而取决于你怎么实现它。 
