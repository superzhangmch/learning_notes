...
如上图，顺序一致大约可理解为每个处理器按code顺序独占式地进行内存访问；但是每个cpu会被随机选中；于是code内有序；整体有某种不确定的序。线性一致性要求read操作也能及时读到最新值；而顺序一致性则不（对应到上面图就是，线性一致下要求write后马上能读到；而顺序一致性不用马上成功，但是一旦延迟并成功后，所有后续read都是能读到新值的）。
