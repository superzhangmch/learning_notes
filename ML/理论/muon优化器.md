...
$(M M')^{-1/2} M = UV'$ 称之为矩阵的符号函数。 $(..)^{-1/2}$ 定义： 对称矩阵函数 $f(X)$ 的定义是若 $X=P \Lambda P'$ 有正交+对角分解，则 $f(X) := Pf(\Lambda)P'$，这里 $X=MM'$, $f=(.)^{-1/2}$。按此定义，则易知 $(M M')^{-1/2} M = UV'$ 成立。
...
$UV' = (M M')^{-1/2} M=P (\Lambda)^{-1/2} P' M $， 而 $P (\Lambda)^{-1/2}$ 其实就是个分维进行的向量函数，每一维是 $\mathbb{R} \Rightarrow \mathbb{R}$。对单维，可以有泰勒级数展开：

$$t^{-1/2} = 1 - \frac{1}{2}(t - 1) + \frac{3}{8}(t - 1)^2 - \frac{5}{16}(t - 1)^3 + \cdots$$

所以代回去后，就能有 $UV'$ 的展开的表达式。

### 为什么 UVᵀ 是谱范数约束下的梯度下降

但是 UV' 的合理性，只靠直觉是不够的，需要严格的证明。
