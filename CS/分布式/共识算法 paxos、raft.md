...
Follower 心里有个倒计时，一直没收到 Leader 消息就超时，超时了就发起选举。发起选举就是**广播拉票**，其他人各自判断投不投，候选人自己数票，超过半数就直接上任，然后开始发心跳，其他人收到心跳自然就认他是 Leader 了。

收到选举请求后, node 投票不是无脑投，要同时满足三个条件：
- 对方 term 比我大
- 对方日志不比我落后
- 我这轮还没投过票

一旦满足, 那么就回复“我支持你”. 而不需要管有几个人在同时竞选.

**Term**

Term 就是选举轮次，**发起选举时就递增**，不是选出 Leader 才递增。leader 稳定期, term num 不变. Term 的作用是区分新旧，收到更高 term 的消息，就知道有新一轮选举发生了，旧 Leader 看到更高 term 就自动下台。

**心跳**

心跳就是空的数据同步消息，没数据要同步的时候定期发一个空的，告诉 Follower "我还活着，别乱动"。有数据同步的时候，数据消息本身就起到心跳的作用，不需要额外再发。

**网络分裂**

node 数量需要是奇数, 且raft 只支持2分裂, 而不支持3,4分裂. 2分裂, 至少有一个group 是有超半数的nodes的. 分裂之后，哪边节点数凑够了多数，哪边就能选出 Leader 正常服务，凑不够的那边就瘫着等网络恢复。2分裂后，**同时只可能有一份凑够多数**，所以永远不会出现两个 Leader 同时写数据。

这就是为什么推荐奇数个节点，奇数天然保证不会出现两边均等分裂的僵局。

raft/paxos 精妙就在于, 每个节点都是机械地按规则做就行了, 不需要知道全局信息, 不用统筹规划仔细计算, 却完成了全局的一致可靠性.
----
