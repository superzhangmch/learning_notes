...
- MOE 每层为（后 58 层）：
  - 每个专家计算量： 3 × dim × moe_hid = 3 × 7168 × 2048 = 44040192 【silu 激活的 FFN: MLP(x) = W₂(SiLU(W₁ x) ⊙ (W₃ x)) 】
...
- 输出投影 Wo : (head_num × head_dim) = (128 × 128) = 16384 → 7168 : 16384 × 7168 = 117440512 【attn 之后的 output proj】
- 以上61层总共：61*(11010048+37748736+4128768+117440512)=10390011904
...
MLA(MQA 模式）:
```
MLA(MQA 模式）: 全序列算 attn：
- QK'
  - no-rope部分: 128头 × 512 × n² = 65536 n²
  - rope部分：128头 × 64 × n² = 8192n^2
- softmax(.)⋅V: 128头 × 512 × n² = 65536 n²
- 以上61层总共：61 * (65536+8192+65536) * n² =8495104 n²

MLA(MQA 模式）: topK=top-2048 算稀疏注意力：则是上面情形中的一个 n 取2048

- QK'
  - no-rope部分: 65536 * 2048 * n = 134217728 n
  - rope部分：8192 * 2048 * n = 16777216 n
- softmax(.)⋅V: 65536 * 2048 * n = 134217728 n
...
- KV还原：k_lora_rank → n_heads × (qk_nope_head_dim + v_head_dim): 512 → 128头 × (128+128)dim = 32768 : 512 × 32768 = 16777216 n
- QK': 128头 × (128+64)dim × n × n = 24576 n²
- softmax(.)⋅V: 128头 × (128+64)dim × n² = 24576 n²
- 以上61层总共：61 * 16777216 n + 61 * (24576+24576) * n² = 1023410176 n + 2998272 n²
...
indexer
```
- QK': q × k_cache: 64头 × 128 × n² = 8192 n²
- 以上61层总共: 61 * 8192 * n² =499712 n²
```
...
推断 n 位置的计算量：引入indexer后 vs 原始，比率是： (54874079232 + 499712 * n) / (36624596992+8495104 * n). 注意当 n → ∞, 极限是 1/17。加速比确实很可观
